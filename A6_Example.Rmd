...
``````{r, message=FALSE}
my_data <- inner_join(census, areas) %>%
  inner_join(acs_age) %>% 
  inner_join(election) %>%
  inner_join(CO_type) %>%
  inner_join(covid) %>%
...
The dataset includes 3088 counties.
...
I can use a scatterplot (`geom_point()`) to visualize the relationship between the median age of a county and its COVID rate.

```{r, warning=FALSE}
ggplot(my_data, aes(x = med_age_E, y = covid_rate)) +
  geom_point(alpha = 0.5) +
  theme_bw()
```

It does sort of look like higher median ages are associated with lower COVID rates. We can even fit a line through the data to make that relationship easier to see.
...
We can quanitify that relationship using a correlation test. You can use `cor.test()` to calculate the correlations between a pair of continuous variables. Here is the correlation between the COVID rate and median age. 
...
Here is a scatterplot illustrating the relationship between COVID rates and population density. I'll log-transform the x-axis like I did for the population density histogram.

```{r, warning=FALSE, message = FALSE}
ggplot(my_data, aes(x = pop_dens, y = covid_rate)) +
  geom_point(alpha = 0.5) +
  stat_smooth() +
    scale_x_continuous(name = "Population density\n(residents per square mile)",
                     trans = "log",
...
I want to know if Republican-leaning counties have higher COVID rates than Democratic-leaning counties, on average. I can use a scatter plot to visualise this difference as well. I'll set `position = "jitter"` to spread out the values in each category

```{r, warning=FALSE, message = FALSE}
ggplot(my_data, aes(x = majority_vote, y = covid_rate)) +
  geom_point(alpha = 0.1, position = "jitter", ) +
  scale_y_continuous(name = "Number of COVID cases per capita") +
  theme_bw()
```

And I can add a box plot or a violin plot to further illustrate the shape of those distributions. Here's a box plot:
...
I can quantify this difference and check for its statistical significance with a two-sample t-test.
...
How do COVID rates differ by county type?

First I'll visualize the differences with a combination of a scatter plot and a violin plot:

```{r, warning=FALSE, message = FALSE}
ggplot(my_data, aes(x = type, y = covid_rate)) +
  geom_violin() +
  geom_point(alpha = 0.1, position = "jitter", ) +
  scale_y_continuous(name = "Number of COVID cases per capita") +
  theme_bw()
...
The biggest difference between those last two models is that the are testing for two different types of relationships between population density and COVID rates. Here is the linear relationship estimated by Model 3:
...
And here is the logarithmic relationship estimates my Model 4:

```{r}
effect_plot(centered_model_scaled_log, pred = pop_dens, interval = TRUE) +
  scale_y_continuous(name = "Number of COVID cases per capita") +
  scale_x_continuous(name = "Population per square mile",
                     breaks = breaks <- seq(0, 70000, by = 10000),
...
Based on the results of my preferred model above, I see that higher median ages are associated with lower COVID rates. Controlling for median age, Republican counties have higher COVID rates than Democratic ones. Does the relationship between age and COVID rates change depending on the political leanings of a county? I can answer that question by interacting political party with median age in my model.
